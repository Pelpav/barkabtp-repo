...
const upload = multer({
  dest: 'uploads/'
}); // Dossier où les fichiers seront temporairement stockés
...
app.post('/barkabtp-repo/blog', upload.single('image'), (req, res) => {
  ...
  const newBlogPost = {
    id: 1,
    // ID généré ou récupéré de la base de données
    title: title,
    image: image.filename,
    // Nom du fichier image sauvegardé sur le serveur
    author: author,
    description: description,
    createdAt: new Date(),
    // Date de création (automatique côté serveur)
  ...
  saveBlog(blog);
  res.json({
    message: 'Nouvel élément ajouté au blog',
    newItem: newBlogPost,
    blog: blog
  });
});
...
app.put('/barkabtp-repo/blog/:id', (req, res) => {
  ...
  if (index !== -1) {
    ...
    saveBlog(blog);
    res.json({
      message: `Blog item with id ${id} updated successfully`,
      updatedItem: blog[index],
      blog: blog
    });
  } else {
    res.status(404).json({
      error: `Blog item with id ${id} not found`
    });
  }
});
...
app.delete('/barkabtp-repo/blog/:id', (req, res) => {
  ...
  if (index !== -1) {
    ...
    saveBlog(blog);
    res.json({
      message: `Blog item with id ${id} deleted successfully`,
